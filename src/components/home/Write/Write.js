...
//       showAddOptionButton: true,
//       requiresAdditionalInput: false,
//       allowsMultipleAnswers: false, // State for allowing multiple answers
//     },
//   ]);
//   const [showSuccessMessage, setShowSuccessMessage] = useState(false);
...
//         showAddOptionButton: true,
//         requiresAdditionalInput: false,
//         allowsMultipleAnswers: false, // Default value for new questions
//       },
//     ]);
//   };
...
//       const db = getDatabase(app);
//       const newSurveyKey = surveyName; // Use the survey name as the key
...
//           options: questions[i].options.map((option) => option.value),
//           requiresAdditionalInput: questions[i].requiresAdditionalInput,
//           allowsMultipleAnswers: questions[i].allowsMultipleAnswers, // Include this field
//         };

//         // Save each question under the new survey key
//         await set(
//           ref(db, `posts/surveys/${newSurveyKey}/${questionKey}`),
//           questionData
//         );
//       }
...
//               />
//               <label className="ml-2 text-gray-700">
//                 Allow multiple answers (checkboxes)
//               </label>
//             </div>
//           </div>
//           <button
...
export default function Write() {
  const [surveyName, setSurveyName] = useState(""); // State to hold the survey name
  const [questions, setQuestions] = useState([
    {
      question: "",
      options: [
      ...
      showAddOptionButton: true,
      requiresAdditionalInput: false,
      allowsMultipleAnswers: false, // State to handle multiple answers
    },
  ]);
  const [showSuccessMessage, setShowSuccessMessage] = useState(false);
  ...
  const handleAddQuestion = () => {
    setQuestions([
      ...questions,
      {
        ...
        showAddOptionButton: true,
        requiresAdditionalInput: false,
        allowsMultipleAnswers: false, // Default value
      },
    ]);
  };
  ...
  const handleSaveData = async () => {
    ...
    try {
      const db = getDatabase(app);
      const newSurveyKey = push(ref(db, "posts/surveys")).key; // Create a new survey key
