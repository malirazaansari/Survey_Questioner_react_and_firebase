  ...
  const fetchAnswers = async () => {
    try {
      ...
      if (snapshot.exists()) {
        ...
        // Iterate over each student's survey submission
        for (const studentId in answersData) {
          if (answersData.hasOwnProperty(studentId)) {
            const studentData = answersData[studentId];
            const { studentName, studentEmail } = studentData;

            // Iterate over each survey submission by the student
            for (const submissionId in studentData) {
              if (
                studentData.hasOwnProperty(submissionId) &&
                submissionId !== "studentName" &&
                submissionId !== "studentEmail"
              ) {
                const surveyAnswers = studentData[submissionId].answers || {};
                const studentDetails = {
                  studentName,
                  studentEmail,
                  answers: surveyAnswers,
                };
                allAnswers.push(studentDetails);
              }
            }
          }
        }
